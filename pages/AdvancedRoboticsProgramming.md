...
* **[Servo Actuator](https://trc492.github.io/pages/ServoActuator.html)**: With the limited number of motors allowed by FTC, servos become the secondary most important actuator on a robot. The Framework Library provides extended support of a servo in the **TrcServo** class over FIRST provided SDK. It supports translation between logical servo positions (between the value of 0 and 1) to physical positions such as 0 to 180 degrees. Just like motors, it also allows you to invert the direction of the servo movement. In addition, it also provides features to support multiple servos (followers). Most importantly, it allows speed controlling a servo motor so you can control a servo by an analog joystick.
